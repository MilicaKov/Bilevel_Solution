...
#   1) flex_matrix.csv  (all k columns)
...
BASELINE_XLSX = r"C:\Users\HP\source\repos\Bilevel_Optimization\Bilevel_Optimization\European_LV_CSV\Baseline.xlsx"
BEV_PARAM_XLSX = r"C:\Users\HP\source\repos\Bilevel_Optimization\Bilevel_Optimization\European_LV_CSV\BEV_parameters.xlsx"
...
OUT_FLEX_CSV = Path("flex_matrix.csv")              # diagnostics / full Fflex

# -----------------------------
# Load inputs & basic checks
# -----------------------------
baseline = pd.read_excel(BASELINE_XLSX)   # columns: t, charger_id, pbase_kw
...
baseline["t"] = pd.to_numeric(baseline["t"], errors="coerce").astype("Int64")
...
if missing_ids:
    raise KeyError(f"charger_id(s) present in Baseline but missing in BEV_parameters: {sorted(missing_ids)}")
...
# ---------------------------------------
def compute_soc_end_hour(df: pd.DataFrame,
                         params_map: dict,
                         time_step_h: float = 1.0) -> pd.DataFrame:
    """
    Returns a copy of df with column 'Eb_t' = energy at END of hour t.
    Rules:
      - Start from E0 per charger.
      - For t > t_dep: keep last SoC (not used for category when p=0 anyway).
      - If pbase_kw > 0: Eb_t += eta * p * Δ (apply efficiency!)
      - If pbase_kw == 0 and t ≤ t_dep: carry previous Eb_t forward.
      - Never reset to E0 inside the day.
    """
    out = df.sort_values(["charger_id", "t"]).reset_index(drop=True).copy()
    # attach per-row parameters
    ...
    Eb = []
    last_E = {}  # current end-of-hour energy per charger
    ...
    for _, row in out.iterrows():
        ...
        # After departure: keep SoC constant
        if t > t_dep:
            Eb.append(last_E[b])
            continue
...
# -------------------------------------------------------
def compute_shiftability_end_hour(df_with_soc: pd.DataFrame,
                                  params_map: dict) -> pd.DataFrame:
    """
    ...
    # continuous needed hours
    denom = (out["eta"] * out["Pmax"]).replace(0, np.nan)
    dt_cont = (out["E_target"] - out["Eb_t"]) / denom
    dt_cont = dt_cont.clip(lower=0).fillna(np.inf)  # guard divide-by-zero (should not happen)
    dt_min = np.ceil(dt_cont)                        # integer hours needed from (t+1 ..)
    # absent condition
...
# -------------------------------------------------------
def build_flex_matrix(shift_df: pd.DataFrame, K: int | None = None) -> pd.DataFrame:
    if K is None:
...
if __name__ == "__main__":
    ...
    # 4) Diagnostics print (optional; comment out if too verbose)
    ...
    # 5) Save full Fflex to CSV (all k columns)
    Fflex.to_csv(OUT_FLEX_CSV, index=True)
    print(f"\nSaved full flexibility matrix to: {OUT_FLEX_CSV.resolve()}")

    # 6) Build the exact sheet your MILP expects: baseline_flex
    #    Bbase: sum of baseline charging at hour t
    Bbase_series = baseline.groupby("t")["pbase_kw"].sum().astype(float).sort_index()

    # Ensure column k=1 exists; if not, create zeros
    if "k=1" not in Fflex.columns:
        Fflex["k=1"] = 0.0

    # Align index with available t
    ...
    basefx = pd.DataFrame({
        ...
        "Fflex1": f1.values,
        # Start with 0 penalty; can tune later (e.g., urgency weights)
        "wflex": 0.0
    })

    # 7) Write/replace the 'baseline_flex' sheet in data/time_series.xlsx
    #    (keeps other sheets users may have already created)
    mode = "a" if OUT_TIMESERIES_XLSX.exists() else "w"
    with pd.ExcelWriter(OUT_TIMESERIES_XLSX, engine="openpyxl", mode=mode, if_sheet_exists="replace") as xlw:
